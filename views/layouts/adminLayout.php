  ...
  <div class="wrapper">
    <header>
      <span class="header-span">
        ...
        <a href=<?= Url::to(['admin/admin/']); ?> class="logo">
          <h1>F</h1>
          inancier
        </a>
      </span>
      ...
      <div class="wrapper">
        ...
        <main>
          <aside class="aside">
            ...
            <a href="<?= Url::to(['admin/admin/']); ?>" id='news'>
              <?= Html::img('@web/image/news.png', ['alt' => 'Новости']); ?>
              <p>Новости</p>
            </a>
            <a href="<?= Url::to(['admin/curseadmin/']); ?>" id='curse'>
              <?= Html::img('@web/image/curse.png', ['alt' => 'Курсы']); ?>
              <p>Курсы</p>
            </a>
            <a href="<?= Url::to(['admin/wordsadmin/']); ?>" id='words'>
              <?= Html::img('@web/image/words.png', ['alt' => 'Слова']); ?>
              <p>Словарь</p>
            </a>
            <a href="<?= Url::to(['admin/usersadmin/']); ?>" id='user'>
              <?= Html::img('@web/image/people.png', ['alt' => 'Пользователи']); ?>
              <p>Пользователи</p>
            </a>
          </aside>
          <div class="content">
        ...
        <script>
          ...
          let ELEM_USER = document.querySelector('#user');


          let path = window.location.pathname;


          if (path === '/web/admin/admin') {
            ACTIVE_ELEM = 0
          } else if (path === '/web/admin/curseadmin') {
            ACTIVE_ELEM = 1
          } else if (path === '/web/admin/wordsadmin') {
            ACTIVE_ELEM = 2
          } else if ('/web/admin/usersadmin') {
            ACTIVE_ELEM = 3
          }
          ...
          function setActive(ACTIVE_ELEM_NUMBER) {
            ...
            if (ACTIVE_ELEM_NUMBER === 0) {
              ...
              ELEM_WORDS.classList.remove('active');
              ELEM_USER.classList.remove('active');
            } else if (ACTIVE_ELEM_NUMBER === 2) {
              ELEM_NEWS.classList.remove('active');
              ELEM_CURSE.classList.remove('active');
              ELEM_WORDS.classList.add('active');
              ELEM_USER.classList.remove('active');
            } else if (ACTIVE_ELEM_NUMBER === 3) {
              ELEM_USER.classList.add('active');
              ELEM_NEWS.classList.remove('remove');
              ELEM_CURSE.classList.remove('active');
              ELEM_WORDS.classList.remove('active');
            }
          }
